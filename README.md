...
`ansible/inventory` があれば `-i ansible/inventory` として指定し、なければ `localhost` を local connection で実行します。`become: true` に対応するため、`ansible-playbook -vvv --ask-become-pass` を使用します。`-vvv` により task/module の実行詳細と command の標準出力・標準エラーを表示します。
